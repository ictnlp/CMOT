...
def process(args):
    ...
    for lang in MUSTC.LANGUAGES:
        ...
        for split in MUSTC.SPLITS:
            ...
            if is_train_split:
                save_df_to_tsv(df, cur_root / f"{split}_{args.task}_{audio_type}_joint.tsv")
            else:
                save_df_to_tsv(df, cur_root / f"{split}_{args.task}_{audio_type}.tsv")

        v_size_str = str(args.vocab_size)
        spm_filename_prefix = f"spm_{args.vocab_type}{v_size_str}_{args.task}"
        spm_model = spm_filename_prefix + ".model" if args.spm_model is None else args.spm_model
        spm_dict = spm_filename_prefix + ".dict" if args.spm_dict is None else args.spm_dict

        # Generate vocab
        if args.spm_model is None:
...
def main():
    ...
    parser.add_argument("--spm-dict", type=str, default=None)
    parser.add_argument(
        "--vocab-type",
        default="unigram",
        type=str,
    ...
    parser.add_argument("--use-zipped", action="store_true")
    parser.add_argument(
    ...
    parser.add_argument("--yaml-filename", type=str)
