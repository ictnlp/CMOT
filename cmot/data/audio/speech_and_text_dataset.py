...
@dataclass
class SpeechAndTextDatasetItem(object):
    index: int
    audio: torch.Tensor
    source: torch.Tensor
    target: torch.Tensor
...
class SpeechAndTextDataset(FairseqDataset):
    ...
    def __getitem__(self, index: int) -> SpeechAndTextDatasetItem:
        ...
        return SpeechAndTextDatasetItem(
    ...
    def collater(
        self, samples: List[SpeechAndTextDatasetItem], return_order: bool = False
    ) -> Dict:
        if len(samples) == 0:
            return {}
...
class SpeechAndTextDatasetCreator(object):
    # mandatory columns
    ...
    @classmethod
    def get_size_ratios(
        cls, datasets: List[SpeechAndTextDataset], alpha: float = 1.0
    ) -> List[float]:
